...
class CallbackEvent(BaseModel):
    ...
    def get_retrieval_message(self) -> dict | None:
        if self.payload:
            ...
            if nodes:
                msg = f"Foram encontradas {len(nodes)} fontes para o uso de contexto da query"
            else:
                msg = f"Encontrando contexto para a query: '{self.payload.get('query_str')}'"
            return {
                "type": "events",
                "data": {"title": msg},
            }
    ...
    def get_tool_message(self) -> dict | None:
        ...
        if func_call_args is not None and "tool" in self.payload:
            ...
            return {
                "type": "events",
                "data": {
                    "title": f"Chamando ferramenta: {tool.name} dos inputs: {func_call_args}",
                },
            }
    ...
    def to_response(self):
        try:
            ...
            logger.error(f"Erro na conversão de evento em resposta: {e}")
            return None
